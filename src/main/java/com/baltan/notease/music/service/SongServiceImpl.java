...
public class SongServiceImpl implements SongService {
     ...
     */
    @Override
    public Map<String, Object> searchAlbum(Map<String, Object> params) {
        Map<String, Object> response = new HashMap<>();
        int responseCode = Response.SUCCESSFUL.getCODE();
        String responseMessage = Response.SUCCESSFUL.getMESSAGE();
        ...
        try {
             ...
             * 1：单曲；10：专辑；100：歌手；1000：歌单；1002：用户
             */
            requestParams.put("type", SearchType.ALBUM.getVALUE());
            /**
             * 页码偏移量
             */
            requestParams.put("offset", offset);
            ...
            paramsMap.put("params", paramArray[0]);
            paramsMap.put("encSecKey", paramArray[1]);
            String json = HttpUtil.post(paramsMap, neteaseConfig.getSearchAlbumsRequestUrl());
            response = ResponseParseUtil.searchAlbumsParse(json);
        } catch (QueryFailureException e) {
            responseCode = e.getCode();
            responseMessage = e.getMessage();
            e.printStackTrace();
